...
fn count_asteroids(pos: (usize, usize), asteroids: &Vec<(usize, usize)>) -> usize {
	asteroids
		.iter()
		.map(|(x, y)| {
			(((*y as f64) - (pos.1 as f64)).atan2((*x as f64) - (pos.0 as f64)) * 1000000.0) as i64
		})
		.collect::<HashSet<i64>>()
		.len()
}
...
fn sorted_asteroids(
	...
	let mut angle_dist = asteroids
		.iter()
		.map(|(x, y)| ((*x as f64) - (pos.0 as f64), (*y as f64) - (pos.1 as f64)))
		.map(|(x, y)| (wrap_angle((x).atan2(-y)), x * x + y * y))
		.zip(asteroids.iter().map(|&x| (x.0, x.1)))
		.collect::<Vec<((f64, f64), (usize, usize))>>();
	angle_dist.sort_by(|x, y| {
		x.0 .0
			.partial_cmp(&y.0 .0)
			.unwrap()
...
fn main() -> Result<(),ureq::Error> {
	...
	let asteroids: Vec<(usize, usize)> = input
		.lines()
		.enumerate()
		.map(|(y, line)| {
			line.unwrap()
	...
	let best_asteroid = asteroids
		.iter()
		.max_by_key(|&x| count_asteroids(*x, &asteroids))
		.unwrap();
