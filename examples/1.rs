...
fn main() -> std::io::Result<()>{
	...
	let mass=input.lines().map(|l|l.unwrap().parse::<u64>().unwrap()).collect::<Vec<u64>>();
	println!("First result: {}",mass.iter().map(|&m|fuel_need(m)).sum::<u64>());
	println!("Second result: {}",mass.iter().map(|&m|total_fuel_need(m)).sum::<u64>());
	Ok(())
}
