...
impl Moon {
	pub fn from_line(l:&str)->Moon{
		...
		let mut it=l[..l.len()-1].split(|x|x==',').map(|x| x.split_once('=').unwrap().1.trim()).map(|x|x.parse().unwrap());
		let pos=Position{x:it.next().unwrap(),y:it.next().unwrap(),z:it.next().unwrap()};
		Moon{pos,vel}
	}
...
fn main() -> std::io::Result<()>{
	...
	let moons0:Vec<Moon>=input.lines().map(|x| Moon::from_line(&x.unwrap())).collect();
