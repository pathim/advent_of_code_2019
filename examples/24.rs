...
fn generate_bitmask(bit:u8)->u32{
	...
	let deltas=[-5,-1,1,5];
	for d in deltas{
		if bit%5==0 && d==-1{continue}
		if bit%5==4 && d==1{continue}
		let bitpos=(bit as i32)+d;
		if bitpos<0 || bitpos >=25{continue;}
		res|=1<<bitpos;
	}
	res
}
...
fn next_generation(current:u32,bitmasks:&Vec<u32>)->u32{
	...
	for (i,m) in bitmasks.iter().enumerate(){
		...
		res|=if (bit!=0 && popcount==1) || (bit==0 && (popcount==2 || popcount==1)){1}else{0}<<i;
	}
	res
}
...
fn main() -> std::io::Result<()>{
	...
	for lmax in 0..200{
		...
		for li in -(lmax+1)..=lmax+1{
			let new_layer=next_generation_fractal(*cl.get(&li).unwrap_or(&0), *cl.get(&(li+1)).unwrap_or(&0), *cl.get(&(li-1)).unwrap_or(&0), &bitmasks);
