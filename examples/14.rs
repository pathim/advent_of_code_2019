...
fn get_needed(
	...
	let actual_amount = if to_produce % r.amount != 0 {
		to_produce - (to_produce % r.amount) + r.amount
	} else {
		to_produce
	};
	...
	r.sources
		.iter()
		.map(|(sn, sa)| get_needed(&sn, sa * multiple, reactions, waste))
		.sum()
}
